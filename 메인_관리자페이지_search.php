...
<?php 
	...
	$block = 5; // 한 페이지에 보여줄 블록 갯수 
        ...
        $sql = "SELECT count(*) as total FROM Profile WHERE id LIKE '%".$search."%' ";
...
<?php
	...
	if($start==0){$board_num=0;}
        ...
        $sql2 = "SELECT * FROM Profile WHERE id LIKE '%".$search."%' order by num desc limit $board_num, $list";
...
if($id == "admin"){echo "<br><br><a href='./메인_관리자페이지.php'>관리자페이지</a></div>";}
			...
			<div class="list_num">
			                    ...
			                    <a href="메인_관리자페이지_search.php?search=<?php echo $search;?>&page=<?php if($s_page==1){echo 1;}else{echo $s_page-1;}?>">이전</a>
	                ...
	                //이전은 스타트 아래로 가면 이전 블록이 되고, 다음은 엔드 위로 가면 다음 블록이 된다.
	                 ...
	                 echo"<a href='메인_관리자페이지_search.php?search=$search&page=$p'>$p</a> ";

	                 }

	                ?>

                      <a href="메인_관리자페이지_search.php?search=<?php echo $search;?>&page=<?php if($e_page<$pageNum){echo $e_page+1;} else if($e_page==$pageNum){echo $pageNum;} ?>">다음</a>			
